...
description: >
  10-step pipeline state reconstruction workflow. Used exclusively by the Orchestrator agent when prior artifacts exist for a feature. Not invoked directly by users.
...
| Tester | `<FEATURE_NAME>_LLM_EVAL_RESULTS.md` | `eval_score` (numeric), `eval_threshold` (numeric), and `eval_status` fields all present; `eval_status` reads exactly `PASS` or `FAIL` (uppercase, exact string match — do not evaluate surrounding prose as a substitute for this field). If the file is absent and the architecture document contains no LLM inference components (no LLM model files in Section 3, no LLM framework in Section 9), classify as complete (skipped — no LLM components). If the Tester phase override flag was set in Step 2, this file is classified as incomplete regardless of its content. |
