...
**Which agent to use:** Run this prompt with a **general agent** — not the Orchestrator. Pipeline analysis is a meta-task: evaluating the pipeline, not running it. The Orchestrator's role is pipeline execution and delegation; using it here conflates its evaluation-subject role with its executor role. A general agent with `Read` and `Grep` tool access is sufficient. No `Agent` invocation tool is required because this prompt produces a report, not a pipeline run.
